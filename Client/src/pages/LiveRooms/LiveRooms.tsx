...
function LiveRooms() {
  ...
  useEffect(() => {
    ...
    // Load messages when switching between already-joined rooms
    ...
    socket.on("dm:new", (incoming: DirectMessageResponse) => {
      ...
      const isViewingThisConversation = dmTarget?.userId === otherUserId;
      ...
      // Always add message to current view if viewing this conversation
      if (isViewingThisConversation) {
        setDmMessages((prev) => [...prev, incoming]);
      }
      
      // Always update the conversation in the left panel
      setDmConversations((prev) => {
        const updated = new Map(prev);
        
        // Create conversation entry if it doesn't exist
        if (!updated.has(otherUserId)) {
          // Try to use existing user data if available from dmTarget or create placeholder
          const userInfo = dmTarget?.userId === otherUserId ? dmTarget : {
            userId: otherUserId,
      ...
      // 1. Current user is receiving the message (sender is someone else)
      // 2. User is NOT currently viewing this conversation
      if (isCurrentUserReceiver && !isViewingThisConversation) {
        setUnreadDmCount((prev) => {
          const next = new Set(prev);
          next.add(otherUserId);
          return next;
        });
      } else if (isViewingThisConversation && isCurrentUserReceiver) {
        // If viewing the conversation and receiving, clear the unread badge immediately
        setUnreadDmCount((prev) => {
          const next = new Set(prev);
      ...
      // Persist unread count to localStorage for sender/receiver tracking
      setUnreadDmCount((prev) => {
        localStorage.setItem("unreadDmCount", JSON.stringify([...prev]));
        return prev;
      });
    });
  ...
  return (
    <section className="min-h-[calc(100vh-88px)] bg-slate-900 flex flex-col">
      <div className="flex-1 grid grid-cols-1 overflow-hidden md:grid-cols-[320px_1fr]">
          <aside className="border-r border-slate-700 bg-slate-800 overflow-hidden flex flex-col">
            ...
            {showCreate && (
              <div className="border-b border-slate-700 bg-slate-700/50 p-3 space-y-2">
                <input
                  value={roomName}
            ...
            {/* Tabs: Rooms vs DMs */}
            <div className="border-b border-slate-700 bg-slate-800 flex gap-0">
              <button
              ...
              >
                ...
                {unreadDmCount.size > 0 && (
                  <span className="absolute -top-1 -right-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-red-500 text-white text-xs font-bold">
                    {unreadDmCount.size}
                  </span>
                )}
              </button>
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <button
                              onClick={() => {
                                ...
                                // If already joined, load this room's messages
                                const socket = socketRef.current;
          ...
          <main className="bg-white overflow-hidden flex flex-col">
            ...
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto bg-slate-50 p-4 space-y-3">
              {!selectedRoom && !dmTarget ? (
                ...
                <div className="grid h-full place-items-center text-center">
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mb-4">You need to join <strong>{selectedRoom.name}</strong> to see messages</p>
                    <button
                ...
                dmMessages.length === 0 ? (
                  ...
                  dmMessages.map((msg) => {
                    ...
                    const isFromCurrentUser = senderUserId === Number(user?.id);
                    // Use message_text from backend (not text)
